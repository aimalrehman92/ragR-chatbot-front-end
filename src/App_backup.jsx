...
function ChatPanel() {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const ans = normalizeScalar(data.answer);
      setAnswer(ans || '(No answer field returned)');
    } catch (err) {
      console.error(err);
...
function RagasPanel() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (statusMetrics !== 'ok') {
        const msg = normalizeScalar(metricsData.message) || 'RAGAS metrics retrieval failed.';
        throw new Error(msg);
      }
      ...
      console.error(err);
      setError(err.message || 'An unexpected error occurred while computing RAGAS metrics.');
      setSummary(null);
      setNQa(0);
      setPlotPath('');
    } finally {
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'An unexpected error occurred while clearing RAGAS data.');
    } finally {
      ...
      <div className="field">
        ...
        <div className="metrics-box">
          ...
          {!loadingAnalyze && !hasSummary && nQa === 0 && !error && (
            <p className="placeholder-text">
              No RAGAS metrics yet. Collect some Q/A interactions, then click "Analyze".
            </p>
          )}
